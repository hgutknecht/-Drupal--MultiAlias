...
Example: A content editor has a site that has a content type called "Product."  In the field settings there are two fieldgroups, one fieldgroup that displays only when the path is "/products/[title-raw]" and another set of fields that only displays when the path is "/products/drupal/[title-raw]". This way a content editor has a single point of administration for content that resolves at muliple paths.
